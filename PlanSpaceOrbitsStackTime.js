...
import { SafeAreaProvider as ChronoShieldField } from 'react-native-safe-area-context';
import { NavigationContainer as StarRouteMatrix } from '@react-navigation/native';
import { GestureHandlerRootView as FluxGripSanctum } from 'react-native-gesture-handler';
import { createNativeStackNavigator as ForgeStackHelix } from '@react-navigation/native-stack';

import ProductScreen from './UnderPlanetaryTimeSrc/UnderPlanetaryTimeScreens/ProductScreen';
import StormIgniteLoader from './UnderPlanetaryTimeSrc/UnderPlanetaryTimeScreens/UnderPlanetaryTimeLoading';
import SkyrVibeCoreDeck from './UnderPlanetaryTimeSrc/UnderPlanetaryTimeScreens/OrbitsInterstellartimeSpaceOfApp';
import SpiritWalkIntro from './UnderPlanetaryTimeSrc/UnderPlanetaryTimeScreens/UnderPlanetaryTimeOnboarding';

const HelixStackCore = ForgeStackHelix();
// libs
import AsyncStorage from '@react-native-async-storage/async-storage';
import { LogLevel, OneSignal } from 'react-native-onesignal';
import AppleAdsAttribution from '@vladikstyle/react-native-apple-ads-attribution';

const NebulaTransitLattice = () => {
  const LatticeScreen = HelixStackCore.Screen;
  const LatticeNavigator = HelixStackCore.Navigator;

  const warpTransitions = {
    mistDrift: {
      headerShown: false,
  ...
  const INITIAL_URL = `https://light-core-tech.site/`;
  const URL_IDENTIFAIRE = `qQVXzmeN`;
  ...
  OneSignal.initialize('bdd8cb9e-227e-43f3-af89-e124fabb1d7e');
  ...
  const Route = ({ isFatch }) => {
    ...
    if (isFatch) {
      return (
        <LatticeNavigator screenOptions={{ headerShown: false }}>
          <LatticeScreen
            initialParams={{
              responseToPushPermition,
              product: finalLink,
    ...
    return (
      <LatticeNavigator
        initialRouteName="UnderPlanetaryTimeOnboarding"
        screenOptions={{
      ...
      >
        <LatticeScreen
          name="UnderPlanetaryTimeLoading"
          options={{
            ...warpTransitions.mistDrift,
            title: 'Storm Loading',
          }}
        ...
        <LatticeScreen
          ...
          name="OrbitsInterstellartimeSpaceOfApp"
        />

        <LatticeScreen
          component={SpiritWalkIntro}
          name="UnderPlanetaryTimeOnboarding"
          options={{
            ...warpTransitions.lightPulse,
            title: 'Onboard Tubes',
          }}
        />
      </LatticeNavigator>
      ...
      <View style={{ flex: 1, overflow: 'hidden' }}>
        ...
        >
          <Image
            ...
            source={require('./UnderPlanetaryTimeSrc/UnderPlanetaryTimeAssets/UnderPlanetaryTimeImages/1.png')}
            resizeMode="cover"
          />
          <Image
            style={{ width: screenWidth, height: '100%' }}
            source={require('./UnderPlanetaryTimeSrc/UnderPlanetaryTimeAssets/UnderPlanetaryTimeImages/2.png')}
            resizeMode="cover"
          />
        </Animated.View>
      </View>
  ...
  return (
    <StarRouteMatrix>
      {!isLoading ? LoudingImg() : <Route isFatch={route} />}
    </StarRouteMatrix>
  );
};
...
const UnderRootOFStck = () => {
  return (
    <FluxGripSanctum style={{ flex: 1, backgroundColor: '#0D0D0E' }}>
      <ChronoShieldField>
        <NebulaTransitLattice />
      </ChronoShieldField>
    </FluxGripSanctum>
  );
};
